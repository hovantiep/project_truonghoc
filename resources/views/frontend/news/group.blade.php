...
@section('left-content')
    ...
    <!-- Pagination -->
    @if($news_group->lastPage() != 1)
        <ul class="pagination justify-content-center mb-4">
            @if($news->currentPage() != 1)
                <li class="page-item">
                    <a class="page-link " href="{!! $news_group->url($news_group->currentPage() - 1) !!}">&larr;</a>
                </li>
            @endif
            @for($i = 1; $i <= $news_group->lastPage(); $i++)
                <li class="page-item {!! ($news_group->currentPage() == $i) ? 'active' : null !!}">
                    <a class="page-link" href="{{ $news_group->url($i) }}">{{ $i }}</a>
                </li>
            @endfor
            @if($news_group->currentPage() != $news_group->lastPage())
                <li class="page-item">
                    <a class="page-link" href="{!! $news_group->url($news_group->currentPage() + 1) !!}">&rarr;</a>
                </li>
            @endif
        </ul>
    @endif
